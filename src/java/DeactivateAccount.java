...
public class DeactivateAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("UPDATE userdetails SET status = 0 WHERE email=?");
            ps.setString(1, email);
            int i = ps.executeUpdate();
            if (i > 0) {
                out.println("<script>alert(\"User " + email + "   has been Successfully Deactivated !!\");</script>");
                RequestDispatcher rs = request.getRequestDispatcher("Admin");
                rs.include(request, response);
            } else {
                out.println("<script>alert(\"Unable to Process your Request\");</script>");
                RequestDispatcher rs = request.getRequestDispatcher("Admin");
                rs.include(request, response);
            }
        } catch (Exception se) {
